...
Here's a complete example of the sign-in flow in a React component:
...
export function SignInWithStacks({ walletAddress }: { walletAddress: string }) {
    ...
    try {
      ...
      await openSignatureRequestPopup({
        ...
        onFinish: async ({ signature }) => {
          ...
          const { data, error: verifyError } = await authClient.siws.verify({
            message,
            signature,
            walletAddress,
            chainId: STACKS_MAINNET.chainId,
        ...
        onCancel: () => {
          setError("User cancelled signature request");
          setIsLoading(false);
        },
      });
    } catch (err) {
  ...
  return (
    <div>
      <button onClick={handleSignIn} disabled={isLoading}>
        {isLoading ? "Signing in..." : "Sign In with Stacks"}
      </button>
